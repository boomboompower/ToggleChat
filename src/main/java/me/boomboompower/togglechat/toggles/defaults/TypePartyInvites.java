...
public class TypePartyInvites extends ToggleBase {
    ...
    private Pattern expiredPattern = Pattern.compile(
        "The party invite (?<where>\\S{1,4}) (?<rank>\\[.+] )?(?<player>\\S{1,16}) has expired.");
    private Pattern invitePattern = Pattern.compile(
        "(?<rank>\\[.+] )?(?<player>\\S{1,16}) has invited you to join (?<meme>\\[.+] )?(?<meme2>\\S{1,16}) party!");
    private Pattern otherInvitePattern = Pattern.compile(
    ...
    public boolean shouldToggle(String message) {
        if (this.wasLastMessageToggled && containsIgnoreCase(message,
