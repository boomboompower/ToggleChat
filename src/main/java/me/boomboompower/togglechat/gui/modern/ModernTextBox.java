...
public class ModernTextBox extends Gui {
    ...
    public ModernTextBox(
        int x, int y, int width, int height, String noTextMessage, boolean onlyAllowNumbers) {
    ...
    /**
     * Deletes the specified number of words starting at the cursor position. Negative numbers will
     * delete words left of the cursor.
     */
    ...
    /**
     * delete the selected text, otherwsie deletes characters from either side of the cursor. params:
     * delete num
     */
    ...
    /**
     * gets the position of the nth word. N may be negative, then it looks backwards. params: N,
     * position
     */
    ...
    public void mouseClicked(int x, int y, int buttonClicked) {
        ...
        if (this.isFocused && flag && buttonClicked == 0) {
            ...
            String s =
                this.fontRendererInstance.trimStringToWidth(
                    this.text.substring(this.lineScrollOffset), this.getWidth());
            this.setCursorPosition(
                this.fontRendererInstance.trimStringToWidth(s, i).length() + this.lineScrollOffset);
        }
    }
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            boolean modern = ToggleChatMod.getInstance().getConfigLoader().isModernTextbox();

            if (modern || this.forceOldTheme) {
                drawRect(
                    this.xPosition - 1,
                    this.yPosition - 1,
            ...
            if (!alertMessage.isEmpty()) {
                this.fontRendererInstance.drawString(
                    ...
                    ((this.xPosition + this.width / 2)
                        - fontRendererInstance.getStringWidth(alertMessage) / 2),
            ...
            if (s.isEmpty() && !isFocused && isEnabled && modern) {
                this.fontRendererInstance.drawString(
                    noTextMessage,
                    ((this.xPosition + this.width / 2)
                        - fontRendererInstance.getStringWidth(noTextMessage) / 2),
                    this.yPosition + this.height / 2 - 4,
                    i,
                    false);
                return;
            ...
            if (flag1) {
                if (flag2) {
                    Gui.drawRect(
                        k1, i1 - 1, k1 + 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT, -3092272);
                } else {
        ...
        if (this.fontRendererInstance != null) {
            ...
            if (pos == this.lineScrollOffset) {
                this.lineScrollOffset -=
                    this.fontRendererInstance.trimStringToWidth(this.text, j, true).length();
            }
    ...
    public void alert(String message, int time) {
        ...
        new Timer().schedule(new TimerTask() {
                    @Override
                    public void run() {
                        ModernTextBox.this.alertMessage = "";
                        ModernTextBox.this.running = false;
                    }
                },
                time);
    }
