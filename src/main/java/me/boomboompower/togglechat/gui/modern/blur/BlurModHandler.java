...
public class BlurModHandler {
    ...
    public void onGuiChange(final GuiOpenEvent event) {
        ...
        if (this.mc.theWorld != null) {
            ...
            if (this.theMod.getConfigLoader().isModernBlur() && !er.isShaderActive() && event.gui != null && event.gui instanceof ModernGui) {
    ...
    public void onRenderTick(final TickEvent.RenderTickEvent event) {
        if (event.phase == TickEvent.Phase.END && this.mc.currentScreen != null && this.mc.entityRenderer.isShaderActive()) {
    ...
    private float getProgress() {
        return Math.min((System.currentTimeMillis() - this.start), 1.0f);
    }

    private void loadShader(EntityRenderer er, ResourceLocation shader) {
        try {
            ReflectionHelper.findMethod(EntityRenderer.class, er, new String[] {"func_175069_a", "loadShader"}, ResourceLocation.class).invoke(er, shader);
